...
module.exports = class APRSClient extends EventEmitter {
	...
	connect() {
		return new Promise((resolve, reject) => {
			...
			/**
				 * @description Emitted when the client connects to the server.
				 *
			...
			/**
				 * @description Emitted when the client receives data from the server.
				 *
				 * @event APRSClient#data
				*/
			...
			/**
			 * @description Emitted when an error occurs while connecting to or receiving data from the server.
			 *
			...
			/**
			 * @description Emitted when the connection times out due to inactivity. Restart connection if needed.
			 *
			 * @event APRSClient#timeout
			 */
			this.socket.on('timeout', (error) => {
				this.emit('timeout', `Timeout from ${this.host}: ${error}`);
			});

			/**
			 * @description Emitted when the connection to the server is disconnected by server.
			 *
			...
			/**
			 * @description Emitted when the APRS client fully closd the connection.
			 *
	 ...
	 */
	sendMessage(message) {
		message = `${this.callsign}>${APRSDEST},TCPIP*:${message}`;
	...
	sendPositionReport(positionReport) {
		...
		let message = `!${latDeg.toString().padStart(2, '0')}${latMin.toFixed(2).toString().padStart(5, '0')}${latHemisphere}${symbolTable}${longDeg.toString().padStart(3, '0')}${longMin.toFixed(2).toString().padStart(5, '0')}${longHemisphere}${symbolCode}`;
	...
	sendAprsWeatherReport(weatherReport) {
		...
		const t = temperature !== null ? (temperature * 1.8 + 32).toFixed(0).padStart(3, '0') : '...';

		// c is wind direction in degrees, rounded to the nearest integer.
		const c = windDirection !== null ? windDirection.toFixed(0).padStart(3, '0') : '...';

		// s is sustained winds in mph
		const s = windSpeed !== null ? (windSpeed / 1.609).toFixed(0).padStart(3, '0') : '...';

		// g is Gust (peak winds in last 5 minutes) in mph
		const g = windSpeedGust !== null ? (windSpeedGust / 1.609).toFixed(0).padStart(3, '0') : '...';

		// h is humidity in percent. 00=100
		let h = humidity !== null ? humidity.toFixed(0).padStart(2, '0') : '..';
		if (h === '100') h = '00';

		// b is Baro in tenths of a mb
		const b = pressure !== null ? (pressure * 10).toFixed(0).padStart(5, '0') : '.....';

		// r is Rain per last 60 minutes in hundredths of an inch, rounded to the nearest integer.
		const r = rainLastHour !== null ? ((rainLastHour / 25.4) * 100).toFixed(0).padStart(3, '0') : '...';

		// p is precipitation per last 24 hours (sliding 24 hour window) in hundredths of an inch, rounded to the nearest integer.
		const p = rainLast24Hours !== null ? ((rainLast24Hours / 25.4) * 100).toFixed(0).padStart(3, '0') : '...';

		// P is precipitation since midnight (local time) in hundredths of an inch, rounded to the nearest integer.
		const P = rainSinceMidnight !== null ? ((rainSinceMidnight / 25.4) * 100).toFixed(0).padStart(3, '0') : '...';
		...
		const positionReport = `${latDeg.toString().padStart(2, '0')}${latMin.toFixed(2).toString().padStart(5, '0')}${latHemisphere}${symbolTable}${longDeg.toString().padStart(3, '0')}${longMin.toFixed(2).toString().padStart(5, '0')}${longHemisphere}${symbolCode}`;
		let message = `@${dom}${hour}${minute}z${positionReport}${c}/${s}g${g}t${t}r${r}p${p}P${P}b${b}h${h}`;
		message += `${comment ? ` ${comment}` : ''}`;
