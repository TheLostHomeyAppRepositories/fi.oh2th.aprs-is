...
module.exports = class APRSClient extends EventEmitter {
  ...
  connect() {
    return new Promise((resolve, reject) => {
      ...
      this.socket.on('end', (error) => {
        /**
         * Emits when the connection to the server is closed.
         *
      ...
      this.socket.on('close', () => {
        /**
         * Emitted when the APRS client is disconnected from the server.
         *
  ...
  sendMessage(message) {
    ...
    if (this.socket) {
      this.socket.write(`${this.callsign}>APZHMY,TCPIP*:${message}\r\n`);
    } else {
  ...
  sendAprsWeatherReport(weatherReport) {
    ...
    if(windSpeed) {
      windSpeed = Math.round(windSpeed).toString().padStart(3, '0');
    } else {
      windSpeed = '...';
    }

    if(windSpeedGust) {
      windSpeedGust = Math.round(windSpeedGust).toString().padStart(3, '0');
    } else {
      windSpeedGust = '...';
    }
